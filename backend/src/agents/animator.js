...
class PythagorasTheorem(Scene):
    ...
    // Generic template — escape concept for Python string context
    const safeConcept = this.escapePython(concept);
    return `from manim import *

class ConceptExplainer(Scene):
    def construct(self):
        title = Text("${safeConcept}", font_size=36)
        title.to_edge(UP)
        self.play(Write(title))
        self.wait(1)
  ...
  getHTML5Template(concept, subject) {
    const safeConcept = this.escapeHtml(concept);
    // Escape for use inside JS string literal (prevent injection via closing quotes/script tags)
    const jsConcept = concept.replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/"/g, '\\"').replace(/<\//g, '<\\/');
    return `<!DOCTYPE html>
<html><head><style>
...
<body>
<div>
  <div class="title">${safeConcept}</div>
...
animate();
  ...
  /**
   * Escape a string for safe inclusion in Python string literals (single-quoted).
   * Prevents injection via backslashes, quotes, and newlines.
   */
  escapePython(text) {
    if (!text) return '';
    return text
  ...
  /**
   * Escape HTML to prevent XSS in generated templates
   */
  escapeHtml(text) {
    if (!text) return '';
    return text
